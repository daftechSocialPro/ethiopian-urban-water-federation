...
import { urlForum, assetUrl } from 'src/endpoints'
...
function Forum({ user }) {
  ...
  // if (connection) {
  //   connection.on('getForum', (result) => {
  //     setForum(result)
  //     customToast('Forum Updated', 0)
  //   })
  // }
  ...
  const currentTableData = useMemo(() => {
    ...
    return Forum.slice(firstPageIndex, lastPageIndex)
  }, [currentPage, Forum])
  ...
  return (
    <>
      <CModal size="xl" visible={visibleXL} onClose={() => setVisibleXL(false)}>
        ...
        >
          <CModalTitle>Forum Public View </CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CCardBody>
            <MDBRow>
              ...
              <MDBCol lg="12">
                <MDBCard className="mb-4">
                  <MDBCardBody>
                    <h4 style={{ fontWeight: 'bold' }}>{ne.title} ({ne.isForumEvent===0?"Forum":"Event"})</h4>
                    <hr />
                    <h6>
        ...
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader className="" style={{ backgroundColor: '#1e4356', color: '#fff' }}>
              <CRow>
                <CCol sm={10}>
                  <strong>Forum</strong> <small>List</small>
                </CCol>
